...
fn test_memory_leak_placeholder() {
    ...
    println!("Allocated leaves: {}, Actual leaves in tree: {}", allocated, actual_leaves);
    
    // This will show the memory leak if it exists
    assert!(allocated >= actual_leaves, "Should have at least as many allocated as in tree");
    
    // The test will reveal the issue by showing excessive allocation
    if allocated > actual_leaves {
        println!("POTENTIAL MEMORY LEAK: {} allocated but only {} in tree structure", 
...
fn test_odd_capacity_split() {
    ...
    for &size in &leaf_sizes {
        if size > 0 && size < min_keys {
            panic!("Split created underfull leaf: {} keys < {} minimum", size, min_keys);
        }
    }
}
...
fn test_linked_list_integrity() {
    ...
    assert_eq!(items_via_iteration, items_via_tree, 
    ...
    for i in 1..items_after_delete.len() {
        assert!(items_after_delete[i-1] < items_after_delete[i], 
...
fn test_range_excluded_bounds() {
    ...
    use std::ops::Bound;
    let items: Vec<_> = tree.range((Bound::Excluded(3), Bound::Unbounded))
        .map(|(k, _)| *k)
        .collect();
    ...
    assert!(!items.contains(&3), "Excluded start bound incorrectly included 3");
    assert!(items.contains(&4), "Should include 4 after excluding 3");
    
    // Test excluded end bound
    let items2: Vec<_> = tree.range((Bound::Unbounded, Bound::Excluded(7)))
        .map(|(k, _)| *k)
        .collect();
    ...
    assert!(!items2.contains(&7), "Excluded end bound incorrectly included 7");
...
fn test_min_keys_consistency() {
    ...
    assert!(test_tree.check_invariants(), "Tree should maintain invariants");
...
fn test_rebalancing_after_deletions() {
    ...
    assert!(tree.check_invariants(), "Tree should maintain invariants after deletions");
...
fn test_arena_utilization() {
    ...
    assert!(utilization > 0.0 && utilization <= 1.0, 
            "Utilization should be between 0 and 1, got {}", utilization);
}