...
fn demonstrate_memory_leak_bug() {
    ...
    if allocated > actual_in_tree {
        println!("✗ BUG CONFIRMED: Memory leak detected - {} extra nodes allocated", 
...
fn demonstrate_incorrect_split_for_odd_capacity() {
    ...
    for &size in &leaf_sizes {
        if size > 0 && size < correct_min_keys {
            println!("✗ BUG CONFIRMED: Leaf has {} keys, should have at least {}", 
...
fn demonstrate_min_keys_inconsistency() {
    ...
    for capacity in [4, 5, 6, 7, 8] {
        ...
        println!("Capacity {}: current={}, correct_leaf={}, correct_branch={}", 
                capacity, current_min, correct_leaf_min, correct_branch_min);
        
        if current_min != correct_leaf_min {
            println!("✗ BUG: Leaf nodes should use {} but use {}", correct_leaf_min, current_min);
        }
    }
}
...
fn demonstrate_range_iterator_excluded_bound_bug() {
    ...
    let items1: Vec<_> = tree.range((Bound::Excluded(5), Bound::Unbounded))
        .map(|(k, _)| *k)
        .collect();
    println!("Range (Excluded(5), Unbounded): {:?}", items1);
    
    // Test excluded start bound where the key doesn't exist
    let items2: Vec<_> = tree.range((Bound::Excluded(6), Bound::Unbounded))
        .map(|(k, _)| *k)
        .collect();
...
fn demonstrate_linked_list_merge_corruption() {
    ...
    // Delete items to trigger merging - this is where the linked list bug occurs
    for i in 8..12 {
        tree.remove(&(i * 10));
    }
    ...
    let expected: Vec<_> = (0..20).filter(|&i| i < 8 || i >= 12).map(|i| i * 10).collect();
    println!("Expected: {:?}", expected);
    
    // The bug in merge_with_left_leaf (line 1025-1030) incorrectly updates
    // left_leaf.next = child_next, which can break the linked list chain
    
    if after != expected {
        println!("✗ BUG CONFIRMED: Linked list corruption detected");
        println!("  Expected: {:?}", expected);
...
fn demonstrate_rebalancing_issues() {
    ...
    let min_keys = 2;
    let leaf_sizes = tree.leaf_sizes();
    let underfull: Vec<_> = leaf_sizes.iter()
        .filter(|&&size| size > 0 && size < min_keys)
        .collect();
    
    if !underfull.is_empty() {
        println!("✗ BUG: Found {} underfull leaves: {:?}", underfull.len(), underfull);
...
fn demonstrate_arena_tree_consistency_issues() {
    ...
    println!("Arena state:");
    println!("  Allocated leaves: {}, Free leaves: {}", allocated_leaves, free_leaves);
    println!("  Allocated branches: {}, Free branches: {}", allocated_branches, free_branches);
    ...
    if allocated_leaves > actual_leaves {
        println!("⚠ POTENTIAL ISSUE: More leaves allocated ({}) than in tree ({})", 
