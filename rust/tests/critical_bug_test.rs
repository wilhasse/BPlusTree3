/// Test to demonstrate the most critical bug: linked list corruption during merges
/// This bug can cause data loss and tree corruption
...
fn test_linked_list_corruption_causes_data_loss() {
    ...
    // Create a specific pattern that will expose the linked list merge bug
    // We need to create a scenario where merge_with_left_leaf is called
    // and the bug in line 1025-1030 corrupts the linked list
    ...
    println!("Items: {:?}", tree.items().map(|(k, _)| *k).collect::<Vec<_>>());
    ...
    // The bug: during merge operations, left_leaf.next = child_next (line 1025-1030)
    // This can break the linked list chain and cause items to become unreachable
    ...
    for &key in &expected_remaining {
        if !tree.contains_key(&key) {
            panic!("CRITICAL BUG: Key {} became unreachable due to linked list corruption", key);
        }
    }
    
    // Check iteration consistency
    if actual_via_iteration != expected_remaining {
        panic!("CRITICAL BUG: Linked list corruption - iteration gives {:?}, expected {:?}",
               actual_via_iteration, expected_remaining);
    }
    
    // If we get here without panicking, the bug might not have manifested in this specific case
    println!("Test passed - linked list corruption not triggered in this case");
}
...
fn demonstrate_memory_leak_accumulation() {
    ...
    for round in 1..=5 {
        ...
        println!("Round {}: {} allocated, {} in tree, {} leaked", 
...
fn test_invariants_after_problematic_operations() {
    ...
    let violations: Vec<_> = leaf_sizes.iter()
        .filter(|&&size| size > 0 && size < correct_min_keys)
        .collect();
    
    if !violations.is_empty() {
        println!("  ✗ Invariant violations: {} leaves below correct minimum", violations.len());
    }
}
...
fn stress_test_arena_consistency() {
    ...
    for cycle in 0..10 {
        ...
        if cycle % 3 == 0 {
            println!("Cycle {}: allocated={}, free={}, in_tree={}", 
    ...
    println!("Final state: {} allocated, {} in tree", final_allocated, final_in_tree);
    
    if final_allocated > final_in_tree {
        println!("  ✗ Final inconsistency: {} extra allocated nodes", 
