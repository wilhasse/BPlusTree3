...
fn test_root_collapse_infinite_loop_attack() {
    ...
    for i in (0..64).rev() {
        if i % 8 != 0 {
            ...
            if let Err(e) = tree.check_invariants_detailed() {
                panic!("ATTACK SUCCESSFUL at deletion {}: {}", i, e);
            }
        }
    }
    ...
    if tree.len() != remaining.len() + 1 {
        panic!("ATTACK SUCCESSFUL: Tree size inconsistent after root collapses!");
    }
}
...
fn test_minimum_capacity_edge_cases_attack() {
    ...
    if let Err(e) = tree.check_invariants_detailed() {
        panic!("ATTACK SUCCESSFUL: Minimum capacity operations failed: {}", e);
    }
}
...
fn test_odd_capacity_arithmetic_attack() {
    ...
    for capacity in vec![5, 7, 9, 11] {
        ...
        if let Err(e) = tree.check_invariants_detailed() {
            panic!("ATTACK SUCCESSFUL with capacity {}: {}", capacity, e);
        }
        ...
        if let Err(e) = tree.check_invariants_detailed() {
            panic!("ATTACK SUCCESSFUL: Root with {} items (capacity {}) invalid: {}", 
                   min_keys, capacity, e);
        }
    }
}
...
fn test_insert_remove_same_key_attack() {
    ...
    let all_items: Vec<_> = tree.items().map(|(k, v)| (*k, v.clone())).collect();
    
    // Check ordering
    for i in 1..all_items.len() {
        if all_items[i-1].0 >= all_items[i].0 {
            panic!("ATTACK SUCCESSFUL: Tree corrupted after rapid insert/remove!");
        }
    }
}
...
fn test_split_merge_thrashing_attack() {
    ...
    for round in 0..10 {
        ...
        if tree.len() != 20 {
            panic!("ATTACK SUCCESSFUL: Lost items during thrashing! Expected 20, got {}", tree.len());
        }
    }
}
...
fn test_extreme_key_values_attack() {
    ...
    let range1: Vec<_> = tree.items_range(Some(&i32::MIN), Some(&0))
        .map(|(k, _)| *k)
        .collect();
    
    if range1.len() != 4 { // MIN, MIN+1, -1000000, -1
        panic!("ATTACK SUCCESSFUL: Range query with MIN bound failed: {:?}", range1);
    }
