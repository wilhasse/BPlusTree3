...
def print_summary(sessions, daily_data):
    ...
    print("=" * 70)
    print(f"Total Programming Time: {total_hours:.1f} hours ({total_minutes:.0f} minutes)")
    ...
    if daily_data:
        ...
        print(f"Programming Days: {len(daily_data)} ({len(daily_data)/total_days*100:.1f}% of days)")
        print()
    
    # Top programming days
    if daily_data:
        top_days = sorted(daily_data.items(), key=lambda x: x[1]['duration_minutes'], reverse=True)[:10]
        print("TOP 10 PROGRAMMING DAYS:")
        for i, (date, data) in enumerate(top_days, 1):
            hours = data['duration_minutes'] / 60
            print(f"  {i:2d}. {date}: {hours:5.1f} hours ({data['commits']:2d} commits, {data['sessions']} sessions)")
        print()
    
    # Longest sessions
    if sessions:
        longest_sessions = sorted(sessions, key=lambda x: x['duration_minutes'], reverse=True)[:10]
        ...
        for i, session in enumerate(longest_sessions, 1):
            ...
            print(f"  {i:2d}. {start_time}-{end_time}: {hours:5.1f} hours ({len(session['commits']):2d} commits)")
        print()
...
def analyze_patterns(sessions, daily_data):
    ...
    for i in range(7):
        ...
        if data['days'] > 0:
            ...
            print(f"  {weekday_names[i]:<9}: {avg_hours:5.1f}h avg ({avg_commits:4.1f} commits avg, {data['days']} days)")

def main():
    print("Analyzing programming time for BPlusTree3 repository...")
