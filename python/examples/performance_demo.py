...
try:
    ...
    print("Note: sortedcontainers not available. Install with: pip install sortedcontainers")
...
def benchmark_range_queries():
    ...
    for data_size in sizes:
        ...
        for range_size in range_sizes:
            ...
            print(f"    B+ Tree:     {bplus_time*1000:.3f} ms ({len(bplus_result)} items)")
            
            # Dict scan approach
            def dict_range():
                return [(k, v) for k, v in regular_dict.items() if start_key <= k < end_key]
            
            dict_time, dict_result = benchmark_function(dict_range)
            print(f"    Dict scan:   {dict_time*1000:.3f} ms ({len(dict_result)} items)")
            ...
            if HAS_SORTEDDICT:
                ...
                print(f"    SortedDict:  {sorted_time*1000:.3f} ms ({len(sorted_result)} items)")
                
                # Performance comparison
                if sorted_time > 0:
                    speedup = sorted_time / bplus_time
                    print(f"    → B+ Tree is {speedup:.2f}x {'faster' if speedup > 1 else 'slower'} than SortedDict")
            
            # Dict comparison
            if dict_time > 0:
                speedup = dict_time / bplus_time
                print(f"    → B+ Tree is {speedup:.2f}x {'faster' if speedup > 1 else 'slower'} than dict scan")
...
def benchmark_memory_usage():
    ...
    print(f"  B+ Tree: ~{sys.getsizeof(bplus_tree) + sum(sys.getsizeof(x) for x in [bplus_tree.keys(), bplus_tree.values()]):,} bytes")
