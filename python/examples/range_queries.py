...
def demo_practical_use_cases():
    ...
    for timestamp, metrics in tree.range(cutoff, None):
        print(f"     {metrics['date']}: {metrics['users']} users, ${metrics['revenue']} revenue")
        count += 1
        if count >= 7:
            break
    ...
    students = [
        ("Alice", 95), ("Bob", 67), ("Charlie", 89), ("Diana", 76),
        ("Eve", 93), ("Frank", 54), ("Grace", 88), ("Henry", 72),
        ("Iris", 91), ("Jack", 63), ("Kate", 85), ("Leo", 79)
    ]
...
def demo_pagination_pattern():
    ...
    print(f"   ... (showing only first 3 pages of ~{len(tree) // page_size} total pages)")
