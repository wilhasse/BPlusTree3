...
fn test_linked_list_cycle_attack() {
    ...
    for i in 0..20 {
        tree.insert(i * 5, format!("initial-{}", i));
    }
    ...
    for round in 0..5 {
        ...
        for (k, _) in tree.items() {
            if !seen.insert(*k) {
                panic!("ATTACK SUCCESSFUL: Linked list has a cycle! Duplicate key: {}", k);
            }
            count += 1;
            if count > tree.len() * 2 {
...
fn test_concurrent_iteration_modification_attack() {
    ...
    for (k, _v) in tree.items() {
        ...
        if let Some(last) = last_key {
            if *k <= last {
                panic!("ATTACK SUCCESSFUL: Iterator returned out-of-order keys: {} after {}", k, last);
            }
        }
        last_key = Some(*k);
    ...
    if iter_count != 50 {
        panic!("ATTACK SUCCESSFUL: Iterator skipped items! Expected 50, got {}", iter_count);
    }
}
...
fn test_split_during_iteration_attack() {
    ...
    for i in 0..10 {
        tree.insert(i * 10, format!("initial-{}", i));
    }
...
fn test_range_iterator_boundary_attack() {
    ...
    let capacity = 5; // Odd capacity for interesting edge cases
    let mut tree = BPlusTreeMap::new(capacity).unwrap();
    ...
    let range1: Vec<_> = tree.items_range(Some(&10), Some(&30)).map(|(k, _)| *k).collect();
    if range1 != vec![10, 15, 20, 25] {
        panic!("ATTACK SUCCESSFUL: Range query returned wrong items: {:?}", range1);
    }
    
    // Test 2: Range with non-existent start key
    let range2: Vec<_> = tree.items_range(Some(&7), Some(&23)).map(|(k, _)| *k).collect();
    if range2 != vec![10, 15, 20] {
        panic!("ATTACK SUCCESSFUL: Range with non-existent start failed: {:?}", range2);
    }
    
    // Test 3: Range that spans exactly one leaf
    let range3: Vec<_> = tree.items_range(Some(&15), Some(&16)).map(|(k, _)| *k).collect();
    if range3 != vec![15] {
        panic!("ATTACK SUCCESSFUL: Single item range failed: {:?}", range3);
    }
    
    // Test 4: Empty range
    let range4: Vec<_> = tree.items_range(Some(&100), Some(&200)).map(|(k, _)| *k).collect();
    if !range4.is_empty() {
        panic!("ATTACK SUCCESSFUL: Empty range returned items: {:?}", range4);
    }
    
    // Test 5: Backwards range (should be empty)
    let range5: Vec<_> = tree.items_range(Some(&30), Some(&10)).map(|(k, _)| *k).collect();
    if !range5.is_empty() {
        panic!("ATTACK SUCCESSFUL: Backwards range returned items: {:?}", range5);
    }
}
...
fn test_linked_list_fragmentation_attack() {
    ...
    for (k, _) in tree.items() {
        ...
        if let Some(prev) = prev_key {
            if *k <= prev {
                panic!("ATTACK SUCCESSFUL: Linked list corrupted! {} <= {}", k, prev);
            }
            
            // Check for large gaps that might indicate missing nodes
            if *k - prev > 100 {
                panic!("ATTACK SUCCESSFUL: Large gap in iteration: {} to {}", prev, k);
            }
        }
    ...
    if count != expected_count {
        panic!("ATTACK SUCCESSFUL: Iterator returned {} items, tree has {}", count, expected_count);
    }
}
...
fn test_iterator_state_corruption_attack() {
    ...
    if items1.len() != 40 {
        panic!("ATTACK SUCCESSFUL: Full iterator wrong length: {}", items1.len());
    }
    
    // Check range iterator 2
    let expected2: Vec<_> = (10..30).map(|i| i * 2).collect();
    if items2 != expected2 {
        panic!("ATTACK SUCCESSFUL: Range iterator 2 wrong: {:?} != {:?}", items2, expected2);
    }
    
    // Check range iterator 3  
    let expected3: Vec<_> = (25..40).map(|i| i * 2).collect();
    if items3 != expected3 {
        panic!("ATTACK SUCCESSFUL: Range iterator 3 wrong: {:?} != {:?}", items3, expected3);
    }
...
fn test_force_linked_list_corruption() {
    ...
    for round in 0..20 {
        ...
        for (k, _) in tree.items() {
            if let Some(l) = last {
                if k <= &l {
                    panic!("ATTACK SUCCESSFUL: Linked list corrupted at round {}", round);
                }
            }
            last = Some(*k);
        }
