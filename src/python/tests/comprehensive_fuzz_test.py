...
def run_capacity_sweep():
    ...
    if failed > 0:
        ...
        for r in results:
            if not r['success']:
                print(f"   Capacity={r['capacity']}, Prepopulate={r['prepopulate']:,}, Seed={r['seed']}")
                if 'exception' in r:
    ...
    for capacity in sorted(capacity_groups.keys()):
        ...
        print(f"   Capacity {capacity:3d}: {avg_rate:6.0f} ops/sec (avg of {len(rates)} tests)")
    
    print(f"\n🏗️  TREE STRUCTURE ANALYSIS:")
    for r in results:
        if r['success'] and r['final_size'] > 0:
            print(f"   Cap={r['capacity']:3d}, Prepop={r['prepopulate']:5,}, Final={r['final_size']:5,}")
...
def run_stress_test():
    ...
    print(f"Configuration: Capacity={capacity}, Prepopulate={prepopulate:,}, Operations={operations:,}")
...
def run_edge_case_tests():
    ...
    for capacity, prepopulate, operations, description in edge_cases:
        print(f"\n🧪 {description}")
        print(f"   Capacity={capacity}, Prepopulate={prepopulate:,}, Operations={operations:,}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        all_passed = (capacity_passed == capacity_total) and stress_passed and edge_passed
