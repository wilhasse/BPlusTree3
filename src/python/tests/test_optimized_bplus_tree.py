...
class OptimizedLeafNode:
    ...
    def _split_and_insert(self, pos: int, key, value) -> Tuple[Any, 'OptimizedLeafNode']:
...
class OptimizedBranchNode:
    ...
    def insert(self, key, right_child) -> Optional[Tuple[Any, 'OptimizedBranchNode']]:
        ...
        if pos < self.num_keys:
            ...
            self.data[start_child + 1:end_child + 1] = self.data[start_child:end_child]
    ...
    def _split_and_insert(self, pos: int, key, right_child) -> Tuple[Any, 'OptimizedBranchNode']:
        """Split branch node."""
...
def test_optimized_performance():
    ...
    for size in sizes:
        ...
        print(f"  Optimized: {optimized_time:.4f}s ({optimized_time/size*1e6:.1f} μs/op)")
        ...
        print(f"  Original:  {original_lookup:.4f}s ({original_lookup/ops_count*1e6:.1f} μs/op)")
        print(f"  Optimized: {optimized_lookup:.4f}s ({optimized_lookup/ops_count*1e6:.1f} μs/op)")
        print(f"  Improvement: {improvement:.1f}%")
