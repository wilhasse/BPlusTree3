        ...
        for i in range(1, 31, 3):
            ...
            if not tree.invariants():
                ...
                if not tree.root.is_leaf():
                    print(f"Root has {len(tree.root.keys)} keys (max: {tree.root.capacity})")
                    print(f"Root has {len(tree.root.children)} children (max: {tree.root.capacity + 1})")
    ...
    def test_specific_deletion_sequence(self):
        ...
        for i, key in enumerate(keys_to_delete):
            ...
            if not valid and not tree.root.is_leaf():
                print(f"  Root: {len(tree.root.keys)} keys, {len(tree.root.children)} children")
                ...
                for j, child in enumerate(tree.root.children[:3]):  # First 3 children
                    if child.is_leaf():
                        ...
                        print(f"  Child {j} (branch): {len(child.keys)} keys, {len(child.children)} children")
                break
                ...
                def examine_node(node, level=0, name="root"):
                    ...
                    if node.is_leaf():
                        ...
                        if len(node.keys) > node.capacity:
                            over_capacity = f" EXCEEDS CAPACITY by {len(node.keys) - node.capacity}"
                        print(f"{indent}{name} (branch): {len(node.keys)} keys, {len(node.children)} children{over_capacity}")
