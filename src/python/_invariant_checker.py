...
class _BPlusTreeInvariantChecker:
    ...
    def check_invariants(self, root: 'Node', leaves: Optional['LeafNode'] = None) -> bool:
        """
        ...
        try:
            if node.is_leaf():
                ...
                for i, child in enumerate(branch.children):
                    if child is None:
                        print(f"Invariant violated: None child at index {i} in _check_keys_ascending")
                        return False
    ...
    def _check_branch_structure(self, node: 'Node') -> bool:
        ...
        if len(branch.keys) != len(branch.children) - 1:
            print(f"Branch structure invalid: {len(branch.keys)} keys but {len(branch.children)} children")
            return False
    ...
    def _get_leaf_depths(self, node: 'Node', depth: int = 0) -> List[Tuple['LeafNode', int]]:
