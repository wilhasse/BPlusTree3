...
fn main() {
    ...
    println!("Tree contains {} items: {:?}", tree.len(), 
    ...
    let range7: Vec<_> = tree.range(custom_range).map(|(k, v)| (*k, v.clone())).collect();
    println!("   (5, 10] = {:?}", range7);
    ...
    let fruit_range: Vec<_> = string_tree.range("cherry".to_string()..="fig".to_string())
